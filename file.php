<?php
  ...
  $pathToFile = $_SERVER['DOCUMENT_ROOT'].'/favicon';

  // максимальный размер файла
  $maxSizeFile = 524288;

  // ваш секретный ключ
  $secret = '6LfzBW0eAAAAAB_Fgyp6y6W3pdoweX0tubYGXwRj';
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    //Телефон
        if (isset($_POST['phone'])) {
          $phone = $_POST['phone'];
          if (!validStringLength($name,2,30)) {
            $data['phone']='Поля имя содержит недопустимое количество символов.';
            $data['result']='error';
          }
        } else {
          $data['result']='error';
  ...
  if ($data['result']=='success') {
    ...
    $mail->CharSet = 'UTF-8';
    $mail->From      = 'info.setonix@yandex.ru';
    $mail->FromName  = 'Имя сайта';
    $mail->Subject   = 'Сообщение с формы обратной связи';
    $mail->Body      = $output;
    $mail->AddAddress( 'info.setonix@yandex.ru' );
